...
- **Full-board (size-G) tilings are exact cover.** Filling every cell forces a perfect tetromino tiling; only ~5% of full-board shape-mixes tile an 8×12 board, and `fullBoardTilingPossible` (checkerboard T-parity) provably rejects ~40% of them for free. The MRV exact-cover tiler resolves each candidate (feasible *or* infeasible) in ~ms, so the widest board now typically **converges to the proven optimum in seconds** (~14s on the reference 58-piece Doomsteed case) rather than coming back `truncated`. Historical caveat now largely moot: pre-MRV the search finished a near-optimal layout fast but kept grinding full-board mixes for minutes without converging, so `truncated` was common even when effectively optimal — and worse, the old tiler could **silently drop a feasible full board** by blowing its node budget, reporting `truncated: false` at a *suboptimal* score. MRV fixed both. If you still see `truncated` on the widest board, it's a genuinely tight budget, not the old "effectively optimal but unconverged" case.
...
- `tiling.ts` — `enumerateDistributions(inventory, totalPieces)` yields shape-count tuples; `tileDistribution(dist, cols, targetLines?, opts?)` finds a line-maximizing packing onto an 8×cols board via an **MRV exact-cover DFS** (`dfsCover`): at each node it branches on the *most-constrained* undecided cell (fewest legal placements), plus a "leave empty" option bounded by `emptyBudget = totalCells − cellsToPlace` (0 for a full board). A cell that can be neither covered nor left empty is a dead end, so infeasibility is proven almost immediately; picking the branch cell by constraint (not by index) preserves completeness — each distinct piece/empty assignment is still visited once. This one path handles full boards (exact cover, `emptyBudget = 0`) and partial boards (G-1/G-2) alike; the old lowest-cell positional DFS is gone. `placementsCoveringCell(shape, cols)` indexes placements by every cell they cover (cached per cols) to support arbitrary branch cells. Capped at `TILE_NODE_BUDGET` DFS visits per call — caller treats null as "no tiling found". `opts: TileOptions` carries an optional `deadline` (abort once `performance.now()` passes it — checked every `DEADLINE_CHECK_INTERVAL` nodes so a single call can't overshoot the solver's time budget) and `nodeBudget` override. `fullBoardTilingPossible(dist, cols)` is a **provable** necessary-condition prefilter for *full-board* distributions (checkerboard coloring ⇒ an odd T-count can't fully tile), applied inside `tileDistribution` to reject ~40% of full-board mixes instantly; it returns `true` (inapplicable) when the distribution leaves cells empty.
...
- `solve.ts` — single-board solver. Looks up the mount via `MOUNTS[mountKey]` at entry, derives `cols`, `tiers`, `boardBits`, `geometricMaxPieces` per call, threads them through. Accepts `pieceBuffMultiplier` (default 1) and `disableLineBonuses` (when true, internally sets `tiers = []` regardless of mount level — used by `solveAll` for non-equipped boards). Two regimes:
  ...
  - K ≥ G: enumerate distributions of size G, G-1, G-2; rank by an admissible upper bound; for each (in rank order) call `tileDistribution` (with `{ deadline }` threaded through) then `selectPieces`. Prune by current best. `mode: 'normal'` adds a fractional-improvement threshold (`normalToleranceEps`). **Ordering: pure descending upper bound.** The optimum almost always lives at the full (size-G) piece count (adding any piece strictly improves the score), and those distributions carry the highest bounds, so they're tried first; finding the true optimum early then upper-bound-prunes the vast majority of G-1/G-2 distributions with *no* tiling attempt. This ordering is only safe because the MRV tiler resolves each distribution (feasible or not) in ~ms. **Historical note:** with the old lowest-cell tiler each full board cost ~341ms to attempt/disprove (~5% feasible), so bound-first ordering spent the entire budget failing to tile full boards before placing a single piece — the empty-Doomsteed bug. The interim fix processed non-full boards first as a workaround; the MRV tiler removed that cost, so we reverted to bound-first (finds the optimum first *and* prunes hardest). If the tiler ever regresses to per-attempt costs like that, revisit this ordering.

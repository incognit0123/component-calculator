...
- `solveAll.ts` — multi-board orchestrator. Validates `mountConfigs` (exactly one `isEquipped`), then optimizes boards in order: equipped first (full buffs, real tiers), then non-equipped by **descending sync rate** (each with `pieceBuffMultiplier = syncRate/100`, `disableLineBonuses: true`). Threads accumulated stats forward so each subsequent solve sees the running diminishing-returns picture. Time budget is **per-board** — each `solve` gets a fresh `timeBudgetMs`, so a truncation on board 1 doesn't starve later boards (they still optimize the leftover inventory under the same budget). `BoardResult.buffsFromPieces` is always **unscaled** (full piece buffs); UI scales by `syncRate` at display time.
